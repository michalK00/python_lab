...
class WRM(App):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Container(id="info"):
            yield Static(AVRG_TIME_RENTAL_TEXT, classes="info-item", id="avrg_time_rental")
            yield Static(AVRG_TIME_RETURN_TEXT, classes="info-item", id="avrg_time_return")
    ...
    def on_data_table_row_selected(self) -> None:
        ...
        self.query_one("#avrg_time_rental", Static).update(AVRG_TIME_RENTAL_TEXT + str(ctrl.get_average_ride_time_on_rental_station(station_name, engine)).split(".")[0])
        self.query_one("#avrg_time_return", Static).update(AVRG_TIME_RETURN_TEXT + str(ctrl.get_average_ride_time_on_return_station(station_name, engine)).split(".")[0])
        self.query_one("#no_bikes", Static).update(NO_BIKES_TEXT + str(ctrl.get_bike_no_on_return_station(station_name, engine)))
        self.query_one("#avrg_time_both", Static).update(AVRG_TIME_BOTH_TEXT + str(ctrl.get_average_ride_time_for_bikes_with_same_stations(station_name, engine)).split(".")[0])
        self.query_one("#no_bikes_both", Static).update(NO_BIKES_BOTH_TEXT + str(ctrl.get_no_bikes_with_same_stations(station_name, engine)))
app = WRM()
if __name__ == "__main__":
    
    engine = create_engine(f"{DBMS_NAME}:///rentals.db")
